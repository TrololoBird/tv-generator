...
class TestEnhancedGenerator:
    ...
    def test_experimental_params_present(self):
        ...
        experimental_params = ["filter2", "sort.sortBy", "sort.sortOrder", "options.decimal_places", "options.currency"]

        for param in experimental_params:
            if param in request_schema["properties"]:
                prop = request_schema["properties"][param]
                assert prop.get("x-experimental") or prop.get(
                    "x-undocumented"
    ...
    def test_gap_coverage(self):
        ...
        for param in gap_params:
            assert param in self.generator.undocumented_params, f"Gap parameter {param} not included in generator"

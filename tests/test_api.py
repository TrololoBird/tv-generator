...
import pytest
from httpx import HTTPStatusError, Request, Response, URL

from src.tv_generator.api import (
    NetworkError,
    RateLimiter,
    RateLimitError,
    SecurityError,
...
class TestTradingViewAPI:
    ...
    async def test_get_field_data(self, api: TradingViewAPI) -> None:
        ...
        data = await api.get_field_data(
            endpoint="america", symbol="AAPL", fields=["name", "close", "volume"]  # Используем symbol, а не ticker
        )
    ...
    async def test_test_field_working(self, api) -> None:
        ...
        tickers = await api.scan_tickers(endpoint="america", label_product="screener-stock", limit=1)

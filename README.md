...
- **Централизованная генерация** - единая точка входа для создания всех спецификаций
...
Настройки проекта находятся в `src/tv_generator/config.py`:
...
│   │   ├── generate_specs.py   # Генерация спецификаций
│   │   ├── cleanup.py          # Очистка проекта
│   │   └── optimize_structure.py # Оптимизация структуры
│   └── utils/                  # Утилиты
│       ├── openapi_updater.py  # Обновление OpenAPI
│       └── validation_and_ci.py # Валидация и CI
├── docs/                       # Документация
...
Логи сохраняются в директории `reports/`:

- `reports/pipeline.log` - логи пайплайна
- `reports/tvgen.log` - логи CLI

...
class TradingViewAPI:
    ...
    def __init__(self):
        ...
        if settings.cookies_path:
            try:
                ...
                if cookies:
                    logger.info(
                        f"Loaded {len(cookies)} cookies from {settings.cookies_path}"
                    )
                else:
                    logger.warning(
                        f"No cookies loaded from {settings.cookies_path}"
                    )
    ...
    def _validate_request_data(self, data: Optional[Dict]) -> None:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if not any(
                    ...
                    for ct in settings.allowed_content_types
                ):
    ...
    async def test_field(
        ...
        try:
            data = await self.get_field_data(
                endpoint, symbol, [field], label_product
            )

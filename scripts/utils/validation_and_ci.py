...
class OpenAPIValidator:
    ...
    def _check_undocumented_parameters(self, spec: Dict[str, Any], 
                                     result: Dict[str, Any]):
                ...
                if (method.lower() == "post" and 
                    ...
                    if self._has_undocumented_params(schema):
                        ...
                        result["warnings"].append(
                            f"Undocumented parameters found in {path}"
                        )
        
        # Проверяем в компонентах
        for schema_name, schema in (spec.get("components", {})
                                   .get("schemas", {}).items()):
            if (schema.get("x-undocumented") or 
                schema.get("x-experimental")):
                undocumented_found = True
                result["warnings"].append(
                    f"Undocumented schema found: {schema_name}"
                )
        
        if undocumented_found:
            result["warnings"].append(
                "Undocumented parameters detected - use with caution"
            )
    ...
    def _has_undocumented_params(self, schema: Dict[str, Any]) -> bool:
        ...
        for prop_name, prop_schema in schema.get("properties", {}).items():
            if (prop_schema.get("x-undocumented") or 
                prop_schema.get("x-experimental")):
                return True
    ...
    def validate_all_specifications(self) -> Dict[str, Any]:
        ...
        valid_specs = sum(1 for result in validation_results.values() 
                         if result["valid"])
